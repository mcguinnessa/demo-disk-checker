...
resource "aws_ecs_task_definition" "diskchecker_task" {
  family                   = "diskchecker-task" # Naming our first task
...
resource "aws_ecs_service" "diskchecker_service" {
  name            = "diskchecker-service"                             # Naming our first service
#  cluster         = "${aws_ecs_cluster.mongo_wrapper_cluster.id}"             # Referencing our created Cluster
  ...
  task_definition = "${aws_ecs_task_definition.diskchecker_task.arn}" # Referencing the task our service will spin up
  launch_type     = "FARGATE"
...
#  depends_on = [aws_lb_listener.listener]
  ...
  network_configuration {
    ...
    security_groups  = ["${aws_security_group.diskchecker_service_security_group.id}"] # Setting the security group
  }
}

resource "aws_security_group" "diskchecker_service_security_group" {
  ingress {
    from_port = 0
    to_port   = 0
    protocol  = "-1"
